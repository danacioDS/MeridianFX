...
def calculate_forecast(pair: str, df: pd.DataFrame, horizon_days: int = 30) -> dict:
    ...
    try:
        ...
        # Intervalos de confianza basados en volatilidad histórica
        historical_vol = calculate_volatility(df, 30) / 100
        horizon_vol = historical_vol * (horizon_days / 252) ** 0.5
        ci_95_lower = current_price * (1 - 1.96 * horizon_vol)
        ci_95_upper = current_price * (1 + 1.96 * horizon_vol)
