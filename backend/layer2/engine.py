...
class DecisionEngine:
    ...
    def _get_policy_diff(self, df_feat, pair: str):
        """Obtiene el policy_diff histórico PIT para el par especificado."""
        try:
            import asyncio
            import pandas as pd
            ...
            def _sync_fetch():
                ...
                try:
                    base_rate = loop.run_until_complete(
                        macro_service.get_historical_policy_rate(base, start_date, end_date)
                    )
                    quote_rate = loop.run_until_complete(
                        macro_service.get_historical_policy_rate(quote, start_date, end_date)
                    )
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                base_rate, quote_rate = executor.submit(_sync_fetch).result()

            if base_rate.empty or quote_rate.empty:
                print(f"⚠️ No hay datos de policy rate para {pair}")
                return None

            policy_diff = MacroDifferentialProvider.calculate_historical(
                base_currency=base,
                quote_currency=quote,
                base_series=base_rate,
                quote_series=quote_rate,
                price_dates=price_dates,
            )

            if len(policy_diff) == 0:
                raise ValueError(f"No se pudo calcular policy_diff {pair}")

            value = float(policy_diff.iloc[-1])
            print(f"📊 PIT policy_diff {pair}: {value:.6f} ({base}={base_rate['policy_rate'].iloc[-1]:.4f}, {quote}={quote_rate['policy_rate'].iloc[-1]:.4f})")
            return value
    ...
    def get_forecast(self, pair: str) -> dict:
        ...
        try:
            ...
            policy_diff_value = self._get_policy_diff(df_feat, pair) or 0.0
