...
| - | ------------------------------------------------------- | -------------------------------------------------------- |
| 1 | **UNAVAILABLE vs NOT_ELIGIBLE resolved**                | Precise definitions for each state                       |
| 2 | **Explainability: Layer 1 does NOT generate narrative** | Narrative comes from Layer 3; Layer 1 only composes      |
| 3 | **Response contracts completed**                        | All endpoints have a defined structure                   |
| 4 | **Lineage: Prediction vs Decision separated**           | Two distinct structures                                  |
...
ELIGIBLE
→ Intelligence exists and can be served.

NOT_ELIGIBLE
→ Intelligence exists but delivery constraints prevent
  actionable exposure (e.g., data quality degraded).

UNAVAILABLE
→ Required intelligence artifacts cannot be retrieved
  or are absent.
...
Executive Narrative comes from Layer 3.
Layer 1 does NOT generate it using an LLM or other means.
...
CACHE:
TTL: 300s
Invalidation: On new prediction
```
...
CACHE:
TTL: 60s
Invalidation: On new ranking cycle
```
...
CACHE:
TTL: 3600s
Invalidation: Never (lineage is immutable)
```
...
**Note:** Cache unhealthy does NOT produce `HALTED`. Cache unhealthy produces `DEGRADED`; the system operates without cache.
...
| ---------------- | -------------------------------------------------------------------------- | --------------------------------- |
| **UNAVAILABLE**  | Required intelligence artifacts cannot be retrieved or are absent.         | HTTP 503 / structured error       |
| **NOT_ELIGIBLE** | Intelligence exists, but delivery constraints prevent actionable exposure. | HTTP 200 with `actionable: false` |
| **ELIGIBLE**     | Intelligence exists and can be served.                                     | HTTP 200 with full forecast       |

### Rules

```text
// Case 1: Intelligence artifacts unavailable
...
**Note:** The mapping of `data_quality.status` to `delivery_state` is a Layer 1 policy. Layer 4 produces the state; Layer 1 determines how that state translates into delivery.
...
| `/v1/status`                        | < 100ms |  < 200ms | Aggregated |
...
| Forecast            | Delivery State       | ELIGIBLE / NOT_ELIGIBLE |
| Status              | System Status (R2)   | Consistent              |
| Status              | SAFE_MODE            | Consumption only        |
| Ranking             | Order                | Layer 2 source          |
| Lineage             | Traceability         | Complete + versioned    |
| Performance         | Calculation          | Layer 1 no calculation  |
...
**Note:** UX, navigation, filters, interaction, and visual hierarchy belong to the Product LLD.

Layer 1 defines **WHAT information is exposed** and **WHICH endpoints provide it**.
...
They define the interfaces, schemas, guarantees, and boundaries through which layers interact.
...
* **UNAVAILABLE** = required artifacts are missing/unavailable.
* **NOT_ELIGIBLE** = intelligence exists but cannot be exposed as actionable.

### 2. ✅ Explainability: Layer 1 does NOT generate narrative

* Narrative comes from Layer 3.
* Layer 1 only composes and formats it.

### 3. ✅ Response contracts completed

* `DriversResponse`
* `PerformanceResponse`
* Two distinct `LineageResponse` structures.

### 4. ✅ Prediction vs Decision lineage separated

* `/lineage/prediction/{id}`
* `/lineage/decision/{id}`
...
| **C**  | Return to Layer 4 — Data Layer, deeper specification              |

**Recommended next step: B — Product LLD**, if the objective is to move from the frozen technical architecture toward the actual user-facing Meridian FX product.
