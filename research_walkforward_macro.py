...
IMPORTANTE:
    ...
    - Mismo modelo para ambos experimentos: LogisticRegression.
"""
...
OUTPUT_FILE = "research_walkforward_macro_results.json"
...
def build_point_in_time_macro(price_index):
    """
    Construye el diferencial USD-EUR point-in-time.
    ...
    Para cada fecha de precio t:
        ...
        policy_diff(t) = USD(t) - EUR(t)

    Nunca usamos una observación macro posterior a la fecha
    de precio.
    """
    ...
    prices = pd.DataFrame({
        "date": price_dates.astype("datetime64[ns]"),
    }).sort_values("date")
    ...
    macro["policy_diff"] = (
        macro["usd_policy_rate"]
        - macro["eur_policy_rate"]
    )
    ...
    macro_audit = pd.merge_asof(
        ...
        eur_dates[["eur_observation_date"]],
        left_on="date",
    ...
    if usd_future.any():
        raise RuntimeError(
            "LEAKAGE DETECTADO: una observación USD "
            "posterior a la fecha de precio fue utilizada."
        )

    if eur_future.any():
        raise RuntimeError(
            "LEAKAGE DETECTADO: una observación EUR "
            "posterior a la fecha de precio fue utilizada."
        )

    print("\n🔍 Auditoría point-in-time")
    print(
        f"   USD future observations: {int(usd_future.sum())}"
    )
    print(
        f"   EUR future observations: {int(eur_future.sum())}"
    )
    ...
    missing_macro = macro[
        ["usd_policy_rate", "eur_policy_rate", "policy_diff"]
    ].isna().any(axis=1)

    print(
        f"   Fechas sin macro completo: "
